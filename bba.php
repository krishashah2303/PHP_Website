...
<body>
    <h2 align="center"><b>NEW BOOKS AVAILABLE ON BOOK SUPPLY HUB</b></h2>
    <br>
    <div class="row">
<?php
include "config.php";
$sql = "SELECT `b_id`, `u_id`, `name`, `author`, `amount`, `image`, `edition`, `bstatus` FROM book where cat_id=2 && bstatus = 'n' order by b_id asc";
          ...
          <form action="cart.php?action=add&bid=<?= $row['b_id']?>" method="post">
            <div class="card">
              ...
              <div>
                ...
                <button class="btn btn-warning flex-fill"><a href="prodescript.php?id=<?php echo $row['b_id']; ?> ">View More</a></button>
                </div>
              </div>
            </div>
          </form>
